// ============================================================================
// SO(3) Utilities (3D Rotation Group)
// ----------------------------------------------------------------------------
// Provides exponential and logarithm maps for SO(3).
// - so3_exp(w): exponential map from axis-angle (deg)
// - so3_ln(m): logarithm map from rotation matrix → axis-angle (deg)
// ----------------------------------------------------------------------------
// Dependencies:
//   - linalg.scad
// ============================================================================
...
// --- Rodrigues Formula (core exp) ------------------------------------------

// Build rotation matrix given axis w and coefficients A, B
...
// Wrapper: exponential map with input in degrees
function so3_exp(w) = so3_exp_rad(w / 180 * PI);

// Exponential map in radians, uses Taylor expansions for small angles
function so3_exp_rad(w) =
  combine_so3_exp(
    w,
    (w * w < 1e-8) ? so3_exp_1(w * w)
    : (w * w < 1e-6) ? so3_exp_2(w * w)
    : so3_exp_3(w * w)
  );

function combine_so3_exp(w, AB) =
  rodrigues_so3_exp(w, AB[0], AB[1]);

// --- Taylor Expansions (near 0) --------------------------------------------

function so3_exp_1(theta_sq) = [1 - theta_sq / 6, 0.5];
...
function so3_exp_3(theta_sq) =
  so3_exp_3_0(sqrt(theta_sq) * 180 / PI, 1 / sqrt(theta_sq));

// --- Logarithm Map ----------------------------------------------------------

// Axis part from skew-symmetric difference
function rot_axis_part(m) =
  [m[2][1] - m[1][2], m[0][2] - m[2][0], m[1][0] - m[0][1]] * 0.5;

// Logarithm: returns axis-angle in degrees
function so3_ln(m) = 180 / PI * so3_ln_rad(m);

// Logarithm in radians
function so3_ln_rad(m) =
  so3_ln_0(
    m,
...
function so3_ln_0(m, cos_angle, preliminary_result) =
  so3_ln_1(
    m, cos_angle, preliminary_result,
...
// --- Symmetric Part Handling (for near-π cases) -----------------------------

function so3_get_symmetric_part_rotation(preliminary_result, m, angle, d0, d1, d2) =
  so3_get_symmetric_part_rotation_0(
    preliminary_result,
    angle,
    so3_largest_column(m, d0, d1, d2)
  );
